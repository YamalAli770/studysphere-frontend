    ...
    <div className="flex flex-col gap-7">
      <div className="w-full flex justify-center">
        ...
        {remotePeers.length>0 ?
          <div>
            {remotePeers[0].roleName?.toLowerCase().replace(/^\w/, (c) => c.toUpperCase())} has already joined
          </div>
        :
          <div>
            You are first to join the room
          </div>
        }
        </div>
      </div>
      ...
      <div className="controls w-full flex flex-row gap-2 justify-center items-center">
        <Button
          ...
          onClick={SwitchVideo}
        >
          {!isLocalVideoEnabled ? <VideoOffIcon size={20} /> : <VideoIcon size={20} />}
        </Button>
        ...
        <Button
          ...
          onClick={SwitchAudio}
        >
          {!isLocalAudioEnabled ? <MicOffIcon size={20} /> : <MicIcon size={20} />}
        </Button>
        <Button onClick={joinMeet} className="bg-white text-black hover:bg-white/70">Join In</Button>
      </div>
    </div>
  );
};
